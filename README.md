...
1. `manualApprovalChoice` Step Functions Choice state checks the decision from the manual review user.
    ...
    1. If approved, run the `moveToScannedDataS3Files` Lambda function.
...
For instrucitons for deployment, refer to the blog post walkthrough [here](https://aws.amazon.com/blogs/security/use-macie-to-discover-sensitive-data-as-part-of-automated-data-pipelines/).
