...
1. `isMacieStatusCompleteChoice` Step Functions Choice state checks whether the Macie sensitive data discovery job is complete.
    i. If yes, the `getMacieFindingsCount` Lambda function runs.
    i. If no, the Step Functions Wait state waits 60 seconds and then restarts Step 5.
...
1. `isSensitiveDataFound` Step Functions Choice state checks whether sensitive data was found in the Macie sensitive data discovery job.
    i. If there was sensitive data discovered, run the `triggerManualApproval` Lambda function.
    i. If there was no sensitive data discovered, run the `moveAllScanStageS3Files` Lambda function.
...
1. `manualApprovalChoice` Step Functions Choice state checks the decision from the manual review user.
    i. If denied, run the `deleteManualReviewS3Files` Lambda function.
    i. If approved, run the `moveToScannedDataS3Files` Lambda function.
